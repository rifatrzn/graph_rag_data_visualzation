...
cypher = """
  MATCH (hp:HealthcareProvider {name: 'Dr. Sarah Johnson'})-[:TREATS]->(p:Patient) 
  RETURN p.name AS PatientName
  """
res = kg.query(cypher)
print("Patients treated by Dr. Sarah Johnson:")
for r in res:
    print(r["PatientName"])
...
# list all patients treated by a specific provider
cypher = """
  MATCH (hp:HealthcareProvider {name: 'Dr. Michael Brown'})-[:TREATS]->(p:Patient) 
  RETURN p.name AS PatientName
  """
res = kg.query(cypher)
print("Patients treated by Dr. Michael Brown:")
for r in res:
    print(r["PatientName"])
